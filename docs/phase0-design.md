...
モデル: **Gemini API 無料枠(Gemini 3 Flash系。モデルIDは設定で差し替え可能にする)**。温度は低め(0.3想定)。出力はJSONのみ(GeminiのJSON出力モードを使用)。プロンプトはプロバイダ非依存の設計とし、`TranslatorGateway` 抽象の背後でモデル・プロバイダを差し替えられるようにする(§6)。
...
## 6. 翻訳AIのAPI: 無料枠の選定とキーの扱い

### 6-1. 無料APIの選定(2026年7月時点の調査)

**要件変更により、AI APIは無料のものを使う。** Anthropic APIには無料枠が存在しない(全モデル従量課金)ため、プロバイダを変更する。

| 候補 | 無料枠 | 日本語品質 | 判定 |
|---|---|---|---|
| **Google Gemini API(Gemini 3 Flash)** | **10リクエスト/分・1,500リクエスト/日**(プロジェクト単位、クレカ登録不要) | ◎ | **採用** |
| Groq(Llama系) | あり | △(日本語の自然さが弱い。「本人っぽさ」が生命線の本製品には不向き) | 予備 |
| OpenRouterの無料モデル | あり(流動的・低レート) | モデル次第 | 予備 |
| Anthropic API | なし | ◎ | 有料化する場合の選択肢 |

夫婦2人で1日数十通の利用に対し1,500リクエスト/日は十分。翻訳呼び出しは `TranslatorGateway` 1点を必ず通すため、将来有料化する場合(Gemini有料枠 or Anthropic)もモデル・プロバイダの差し替えだけで済む。

**⚠️ 無料枠の最重要注意点(正直に明示する)**: Geminiの無料枠では、**送信した内容(プロンプト)と応答がGoogleの製品改善に利用されることがあり、人間のレビュアーが処理する可能性がある**(有料枠は改善利用の対象外)。本アプリは翻訳のために「荒れた原文」をAPIへ送る設計なので、これはユーザーに隠してはならない事実である。対応:
- オンボーディングと設定画面に明記: 「原文は翻訳のためGoogleのAI(Gemini)に送信されます。無料版のAIを使っているため、送信内容がGoogleのサービス改善に使われることがあります。当アプリのサーバーには保存されません」
- 実名・住所など特定につながる固有情報を書かないことを推奨する一文を書き殴りボックス付近に置く
- 商用化・本気運用の段階でGemini有料枠(改善利用なし)への切替を第一の改善項目とする

### 6-2. キーの置き場所: 2案比較(構図は変わらず)

| 観点 | 案A: サーバーレスプロキシ | 案B: ユーザー自身の無料APIキー |
|---|---|---|
| 構成 | Cloudflare Workers にキーを置き、PWA→Workers→Gemini | 設定画面でキー入力、localStorage保存、端末→Gemini直(Gemini APIはブラウザからの直接呼び出しに対応) |
| キーの安全性 | ○ クライアントに一切出ない | △ 端末ローカルのみ。ただし**無料キーなので漏れても金銭被害はない**(無料枠を他人に食われるだけ)。キー再発行も無料 |
| 配布の楽さ | ○ 相手(配偶者)は何もしなくていい | △ Google AI Studioでの無料キー取得(Googleアカウントがあれば数分)。Anthropic時代より大幅にハードルが下がった |
| 運用コスト | Workers無料枠内で0円+API費0円 | 完全0円・サーバー不要 |
| レート枠 | 1キー共有: 10RPM/1,500RPDを2人で分け合う(十分) | 各自のキーで枠が2倍 |
| フェーズ2との関係 | 配達リレーにどのみちWorkersが必要。同居できる | リレーは別途必要だが、**翻訳はプロキシを経由しない分Workersが軽くなる** |

**結論(Anthropic時代から変更)**: 無料キーになったことで案Bのリスクとハードルが大きく下がったため、**フェーズ1〜2は案B(各自の無料Geminiキーを端末ローカル保存)を基本**とする。フェーズ2のWorkersは配達リレー専任にし、翻訳プロキシ(案A)は「配偶者にキー取得すらさせたくない」場合のオプションとして実装する。両対応の抽象化は要件どおり維持:

```ts
interface TranslatorGateway {
  translate(req: TranslateRequest): Promise<TranslateResult>;
}
// 実装1: DirectGateway (端末→Gemini直・自分の無料キー) — フェーズ1のデフォルト
// 実装2: ProxyGateway (Workers経由・キーはWorkersにのみ) — オプション
```

課金・商用化を見据え、翻訳呼び出しはこのGateway 1点を必ず通す(従量部分・プロバイダ差し替えの分離点)。
...
- サーバー(Workers + KV)には**配達待ちのDeliveryPayloadのみ**を保持し、受信側の取得確認後に削除。原文は端末からGemini APIへ直接送信され(案B)、当アプリのサーバーを一切通らない。この旨を設定画面に明記: 「原文は翻訳のためGoogleのAI(Gemini)にのみ送信されます。無料版のAIのため、送信内容がGoogleのサービス改善に使われることがあります。当アプリのサーバーには保存されません」(§6-1参照)。
...
3. **Gemini API無料枠の条件**: レート上限(現時点で Gemini 3 Flash: 10RPM/1,500RPD)と「無料枠は入力をGoogleの製品改善に利用しうる」というデータ利用条件は変わりうる。実装時・リリース前に最新の利用規約を確認する。
4. **モデルID**: Gemini無料枠対象のFlash系最新モデルを設定値として使用(モデルIDは改廃が激しいため定数1箇所+設定画面で差し替え可能にする)。プロンプトはプロバイダ非依存に書き、有料化時のGemini有料枠/Anthropicへの移行に備える。
...
- **フェーズ1**: 1台内MVP。書き殴り→種類選択→翻訳→調整→承認→自分の受信箱に表示、のループ。DirectGateway(自分の無料Geminiキー)で動かす。原文破棄・needs_more・safety_refusedをこの時点で完成させる。
